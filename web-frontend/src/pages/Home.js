import { FiArrowRight, FiBarChart2, FiCpu, FiLock, FiShield, FiTrendingUp, FiZap } from "react-icons/fi";
...
/* ─── Layout ─────────────────────────────────────────────── */
const Page = styled.div`min-height:100vh;background:#0b0e17;overflow-x:hidden;`;
...
const TickerWrap = styled.div`
  margin-top:68px;background:rgba(59,130,246,.08);
...
const TickerItem = styled.div`
  display:flex;align-items:center;gap:8px;
  padding:0 32px;white-space:nowrap;font-size:13px;
  font-family:'DM Mono',monospace;
  span:first-child{color:#94a3b8;}
  span.up{color:#10b981;}
  span.dn{color:#ef4444;}
`;
...
const GlowOrb = styled.div`
  ...
  width:${p => p.$size||'420px'};height:${p => p.$size||'420px'};
...
const HeroTitle = styled.h1`
  font-size:clamp(2.6rem,6vw,5.2rem);font-weight:800;letter-spacing:-.03em;
  line-height:1.08;margin-bottom:24px;max-width:860px;
...
const PrimaryBtn = styled(Link)`
  ...
  transition:all .25s;
  &:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(59,130,246,.55);}
`;
...
const StatBox = styled.div`
  flex:1;padding:24px 20px;text-align:center;background:#111827;
  &:not(:last-child){border-right:1px solid rgba(255,255,255,.06);}
`;
const StatNum = styled.div`
  font-family:'Syne',sans-serif;font-size:1.7rem;font-weight:800;
  color:#f1f5f9;margin-bottom:4px;
`;
const StatLabel = styled.div`font-size:12px;color:#64748b;font-weight:500;letter-spacing:.04em;text-transform:uppercase;`;
...
const SectionSub = styled.p`
  color:#64748b;font-size:1.05rem;max-width:520px;line-height:1.7;
  margin-bottom:60px;
`;
...
const FeatureCard = styled.div`
  ...
  &::before{
    ...
    background:linear-gradient(90deg,${p=>p.$color||'#3b82f6'},transparent);
...
const FIcon = styled.div`
  ...
  margin-bottom:20px;
  svg{color:${p=>p.$color||'#3b82f6'};font-size:22px;}
`;
const FTitle = styled.h3`font-size:1.1rem;font-weight:700;margin-bottom:10px;`;
const FDesc  = styled.p`color:#64748b;font-size:14px;line-height:1.7;`;
...
`;
const StepsInner = styled.div`max-width:1100px;margin:0 auto;`;
...
`;
const StepTitle = styled.h3`font-size:1rem;font-weight:700;margin-bottom:10px;`;
const StepDesc  = styled.p`color:#64748b;font-size:13.5px;line-height:1.7;`;
...
const CTABox = styled.div`
  ...
  border:1px solid rgba(59,130,246,.25);border-radius:24px;
  ...
  &::before{
    ...
    background:linear-gradient(135deg,rgba(59,130,246,.2),rgba(139,92,246,.1));
  }
`;
...
`;
const CTASub = styled.p`color:#94a3b8;font-size:1.05rem;margin-bottom:40px;`;

/* ─── FOOTER ─────────────────────────────────────────────── */
const Footer = styled.footer`
  border-top:1px solid rgba(255,255,255,.06);padding:36px 40px;
  display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;
`;
const FooterLogo = styled.div`
  font-family:'Syne',sans-serif;font-size:18px;font-weight:800;color:#3b82f6;
  span{color:#f97316;}
`;
const FooterCopy = styled.p`color:#334155;font-size:13px;`;
...
const Home = () => {
  ...
  return (
    <Page>
      {/* NAV */}
      <Nav>
        <NavLogo>Option<span>ix</span></NavLogo>
        <NavLinks>
          <NavBtn to="/login">Sign In</NavBtn>
          <NavBtn to="/login" $primary="true">Get Started <FiArrowRight /></NavBtn>
        </NavLinks>
      </Nav>
      ...
      <HeroSection>
        <GridBg />
        <GlowOrb $top="-10%" $left="5%"  $size="500px" $color="rgba(59,130,246,.1)" />
        <GlowOrb $top="20%"  $right="0%" $size="380px" $color="rgba(139,92,246,.08)" />
        <GlowOrb $bottom="0" $left="30%" $size="300px" $color="rgba(6,182,212,.07)" />

        <Badge><Dot /> Live trading platform</Badge>

        <HeroTitle>
          Trade Derivatives with<br />
        ...
        <FeatureGrid>
          {[
            ...
            <FeatureCard key={i} $color={f.$color}>
              <FIcon $bg={f.$bg} $color={f.$color}>{f.icon}</FIcon>
      ...
      <StepsSection>
        <StepsInner>
          ...
          <Steps>
            {[
              {n:"01", title:"Create your account", desc:"Sign up in under 60 seconds. No KYC friction, just a verified email to get started."},
              {n:"02", title:"Connect your portfolio", desc:"Import positions via CSV or link your broker. We support 40+ major brokerages."},
              {n:"03", title:"Analyse & trade", desc:"Explore the live option chain, run risk scenarios, and execute with a single click."},
              {n:"04", title:"Monitor in real-time", desc:"Your dashboard updates live — P&L, Greeks, margin usage, and alert triggers."},
            ].map((s,i) => (
              <Step key={i}>
                <StepNum>{s.n}</StepNum>
      ...
      <CTASection>
        <CTABox>
          ...
          <PrimaryBtn to="/login" style={{display:"inline-flex",margin:"0 auto"}}>
      ...
      {/* FOOTER */}
      <Footer>
        <FooterLogo>Option<span>ix</span></FooterLogo>
        <FooterCopy>© {new Date().getFullYear()} Optionix Inc. All rights reserved.</FooterCopy>
      </Footer>
    </Page>
  );
};
